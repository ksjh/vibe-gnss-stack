...
#
# The ser2net YAML (/etc/gnss/ser2net.yaml) is NOT created by the installer.
# It is generated automatically at startup of gnss-stack via ExecStartPre.
#
...
apt-get install -y --no-install-recommends \
    ser2net python3-venv python3-pip 2>/dev/null || true

# Check ser2net version (requires v4+ for YAML format)
if command -v dpkg &>/dev/null; then
    SER2NET_VERSION=$(dpkg -s ser2net 2>/dev/null \
        | grep '^Version:' | awk '{print $2}' || echo "unknown")
else
    SER2NET_VERSION=$(ser2net -v 2>&1 | head -1 || echo "unknown")
fi
echo "  ser2net version: ${SER2NET_VERSION}"

SER2NET_MAJOR=$(echo "$SER2NET_VERSION" | grep -oP '^\d+' || echo "0")
if [[ "$SER2NET_MAJOR" =~ ^[0-9]+$ ]] && \
   [[ "$SER2NET_MAJOR" -gt 0 ]] && \
   [[ "$SER2NET_MAJOR" -lt 4 ]]; then
    echo "  WARNING: ser2net < v4 – v4+ required (YAML configuration)."
    echo "           apt-get install -t $(lsb_release -cs)-backports ser2net"
fi
...
[Unit]
Description=GNSS Stack (ser2net + str2str)
...
# Step 1: generate ser2net YAML from the current TOML.
# On failure (exit != 0) ExecStart is not run;
# systemd retries automatically after RestartSec.
ExecStartPre=$INSTALL_DIR/venv/bin/python $INSTALL_DIR/str2str_manager.py \\
    --config \${GNSS_CONFIG} --generate-yaml

# Step 2: start ser2net + str2str.
...
echo "║                                                              ║"
echo "║  4. Start the stack (starts ser2net + str2str):              ║"
